...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        for sample_name in data_dicts_region:
            if "_" in sample_name:
                gene, panel = sample_name.split("_", 1)
            else:
                gene = sample_name
                panel = "."
            
            data_dicts_region[sample_name]["gene"] = gene
            data_dicts_region[sample_name]["panel"] = panel
        ...
        log.info(f"Found reports for {len(samples_found)} samples hahahahaha")
